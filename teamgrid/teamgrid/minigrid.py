...
class MiniGridEnv(gym.Env):
    ...
    def reset(self, seed=None, options=None):
        ...
        self.step_count = 0
        self.agent_step_count = [0] * len(self.agents)
    ...
    def step(self, actions):
        ...
        return obss, rewards, False, trunc, {}
    ...
    def render(self, mode='human', close=False):
        ...
        if mode == 'human':
            ...
            for idx, obs in enumerate(obss):
                ###############################################
                # assume that we have screwed up the render image
                # so we need to account for a flattened image
                img = obs[:147]
                img = img.reshape(7, 7, 3)
                ###############################################
                pixmap = self.get_obs_render(img, self.agents[idx].color)
