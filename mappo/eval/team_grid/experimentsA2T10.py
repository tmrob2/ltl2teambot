...
register(
    'LTL10TA2-v0', 
    entry_point='mappo.envs:LTL10TA2',
)

# construct a set of environments

writer = SummaryWriter("runs/T10A2")
...
for i in range(num_procs):
    envs.append(make_env("LTL10TA2-v0", seed))
...
while num_frames < frames and (best_score < 0.95 or mean_agent_costs < 0.95 or determ_alloc < 0.95):#any(i < 0.95 for a in best_scores for i in a):
    ...
    alloc_loss = loss(task_input, task_target) + 1. * loss(cost_input, cost_target)
    alloc_loss.backward()
    alloc_optim.step()
