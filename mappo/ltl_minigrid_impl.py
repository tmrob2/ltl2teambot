from mappo.algorithms.ppo import PPO
import minigrid
...
for i in range(num_procs):
    envs.append(make_env('LTL1Custom-6x6-v0', 1234 + 10000 * i, img_only=False))
...
model = ACLTLModel(envs[0].observation_space, envs[0].action_space, use_memory=True)
model.to(device)

ppo = PPO(envs=envs, acmodel=model, device=device, preprocess_obss=None, 
...
while num_frames < frames and best_score < 0.95:
