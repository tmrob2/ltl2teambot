...
class OUActionNoise(object):
    ...
    def __call__(self):
        # function to introduce exploration because policy is deterministic
...
class ReplayBuffer(object):
    ...
    def sample_buffer(self, batch_size):
        ...
        return states, action, reward, new_states, terminals
...
class CriticNetwork(nn.Module):
    def __init__(
        ...
        self.bn1 = nn.LayerNorm(self.fc1_dims) # batch normalisation
        ...
        torch.nn.init.uniform_(self.q.bias.data)
...
class ActorNetwork(nn.Module):
    def __init__(self, alpha, input_dims, fc1_dims, fc2_dims, n_actions, name, chkpt_dir='tmp/ddpg') -> None:
...
class Agent(object):
    def __init__(
        ...
        self.target_actor = ActorNetwork(alpha, input_dims, layer1_size, 
            layer2_size, n_actions, name='TargetActor')

        self.critic = CriticNetwork(beta, input_dims, layer1_size, layer2_size,
            n_actions, name='TargetCritic')
        ...
        # if you use one network then you are chasing a moving target which can lead to 
        # divergence, estimate the policy with one network and evalulate the policy with 
        # another
    ...
    def update_network_parameters(self, tau=None):
        ...
        target_critic_params = dict(target_critic_params)
